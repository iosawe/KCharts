...
@property (nonatomic, strong) UILongPressGestureRecognizer * _Null_unspecified _longPressGestureRecognizer;
/// 添加的属性 默认以Y轴为中心缩放 设置NO： 以两个手指中心缩放 liulei修改
@property (nonatomic) BOOL _zoomDependYaxis;
- (nonnull instancetype)initWithFrame:(CGRect)frame OBJC_DESIGNATED_INITIALIZER;
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)aDecoder OBJC_DESIGNATED_INITIALIZER;
- (void)initialize;
- (void)observeValueForKeyPath:(NSString * _Nullable)keyPath ofObject:(id _Nullable)object change:(NSDictionary<NSKeyValueChangeKey, id> * _Nullable)change context:(void * _Nullable)context;
...
/// This is to prevent using setTranslation which resets velocity
@property (nonatomic) CGPoint _lastPanTranslation;
@property (nonatomic) BOOL _dragOneCandleWidth;
@property (nonatomic) CGAffineTransform _PanBeganTouchMatrix;
- (void)nsuiTouchesBegan:(NSSet<UITouch *> * _Nonnull)touches withEvent:(UIEvent * _Nullable)event;
...
- (void)resetZoom;
- (void)ll_zoomWithScaleX:(CGFloat)scaleX;
...
/// \param axis 
///
- (void)ll_zoomToCenterWithScaleX:(CGFloat)scaleX scaleY:(CGFloat)scaleY;
/// Zooms to the center of the chart with the given scale factor.
/// \param scaleX if < 1 –> zoom out, if > 1 –> zoom in
///
/// \param scaleY if < 1 –> zoom out, if > 1 –> zoom in
...
- (void)setViewHighlightWithHighlight:(ChartHighlight * _Nullable)highlight;
///
/// returns:
...
/// liulei修改 高亮线跟随手指移动 还是 收盘价
@property (nonatomic) BOOL highlightFollowFinger;
@end
...
- (void)drawHighlightLinesWithContext:(CGContextRef _Nonnull)context point:(CGPoint)point set:(id <ILineScatterCandleRadarChartDataSet> _Nonnull)set leftSpace:(CGFloat)leftSpace;
- (void)drawHighlightLeftLabelWithContext:(CGContextRef _Nonnull)context point:(CGPoint)point set:(id <ILineScatterCandleRadarChartDataSet> _Nonnull)set yValue:(double)yValue;
@end
...
- (void)drawDataWithContext:(CGContextRef _Nonnull)context;
@property (nonatomic) CGRect _lastBodyRect;
...
- (void)chartHighlightChanged:(ChartViewBase * _Nonnull)chartView highlight:(ChartHighlight * _Nullable)highlight;
- (void)chartTaped:(ChartViewBase * _Nonnull)chartView;
@end
...
@property (nonatomic) CGPoint _touchPoint;
...
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)initWithX:(double)x y:(double)y xPx:(CGFloat)xPx yPx:(CGFloat)yPx dataIndex:(NSInteger)dataIndex dataSetIndex:(NSInteger)dataSetIndex stackIndex:(NSInteger)stackIndex touchPoint:(CGPoint)touchPoint axis:(enum AxisDependency)axis OBJC_DESIGNATED_INITIALIZER;
